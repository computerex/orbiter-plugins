			...
			if (oapiGetSimTime()-data->lSampleTime > 1)
			{
				...
				for (int i = 0; i < 17; i++)
				{
					if (data->points[i].x-data->width*3<data->width)
					{
						...
						{
							VECTOR3 hVel;
							v->GetHorizonAirspeedVector(hVel);
							err=fabs(data->v->ap[data->SelectedAP].sp-hVel.y);
						}else if (data->SelectedAP==AP_VEL)
						{
							err = fabs(data->v->ap[data->SelectedAP].sp-v->GetAirspeed());
						}
						p=err/500;
						p=p*(data->height*24);
...
bool TCMFD::ConsumeKeyBuffered (DWORD key)
{
	...
	case PAGE_AP:
		...
		case OAPI_KEY_NUMPAD5:
			{
				if (data->kRotActive==TRUE)
					data->kRotActive=FALSE;
				else
					data->kRotActive=TRUE;
				return true;
			}
		}
		break;
...
bool TCMFD::ConsumeButton (int bt, int event)
{
	...
	case PAGE_AP:
		...
		case 8:
			return ConsumeKeyBuffered(OAPI_KEY_NUMPAD5);
		}
	}
	return false;
}
...
char *TCMFD::ButtonLabel (int bt)
{
	...
	case PAGE_AP:
		{
			char * label[9] = {"PG", "NAP", "GRP", "SP", "TGL", "BDT", "CNT", "STA", "KOT"};
...
void TCMFD::PrintAP(HDC hDC)
{
	...
	y+=height*1;
	if (data->kRotActive==TRUE)
		len = sprintf(cbuf, "Kill Rot        : Active");
	else
		len = sprintf(cbuf, "Kill Rot        : Inactive");
	TextOut(hDC, width, y, cbuf, len);
	y+=height*2;
